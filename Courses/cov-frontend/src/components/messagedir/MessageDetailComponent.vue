      ...
      <div class="container" style="margin-top: 40px;">
        <div class="row">
          <div class="col-6 pt-4 pt-lg-0">
              <h1 class="title mb-4">{{message.title}}</h1>
              <div id="content"></div>
          </div>
          <div class="col-6">
              <img style="margin-top: 50px;" :src="message.imgurl" class="img-fluid" alt="">
          </div>
        </div>
      </div>
...
export default {
  ...
  mounted() {
    ...
    var that = this
    this.postRequest('/knowledge/getknowledge',{id: id})
      .then((response) => {
        that.message = response.data
        this.$nextTick(() => {
          var div = document.getElementById("content");
          div.innerHTML= this.message.content
          div.style.fontFamily = 'rr'
        })
      })
      .catch((error) => {
        console.log(error)
