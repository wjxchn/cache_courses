...
class DomesticcovPipeline(object):
    # 打开数据库
    i = 0
    ...
    def update_db(self, item):
        ...
        try:
            ...
            #     id[0]
            # )
            # sql = 'UPDATE locations SET latitude = %s, longitude = %s WHERE id = %s'
            # cursor = self.db_conn.cursor()
            ...
            sql = 'UPDATE covdata SET confirm = %s, confirm_add = %s, now_confirm = %s, now_confirm_add = %s, dead = %s, ' \
                  'dead_add = %s, heal = %s, heal_add = %s, no_infect = %s, no_infect_add = %s, time = %s WHERE ' \
                  'location_id = %s '
            cursor = self.db_conn.cursor()
            cursor.execute(sql, value)
            self.db_conn.commit()
    ...
    def insert_db(self, item):
        ...
        try:
            sql1 = 'INSERT INTO locations VALUES(%s,%s,%s,%s,%s)'
            sql2 = 'INSERT INTO covdata(location_id, confirm, confirm_add, now_confirm, now_confirm_add, dead, dead_add, ' \
                   'heal, heal_add, no_infect, no_infect_add) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) '
